...
#model_path = './final_models/mcnn_shtechA_232.h5'
...
for epoch in range(start_step, end_step+1):    
    ...
    if (epoch % 2 == 0):
        ...
        if mae < best_mae:
            best_mae = mae
            best_model_mae = '{}_{}_{}.h5'.format(method,dataset_name,epoch)
        if mse < best_mse:
            best_mse = mse
            best_model_mse = '{}_{}_{}.h5'.format(method,dataset_name,epoch)
        if train_loss < best_trainloss:
            best_trainloss = train_loss
            best_model_trainloss = '{}_{}_{}.h5'.format(method,dataset_name,epoch)
        log_text = 'EPOCH: %d, MAE: %.1f, MSE: %0.1f,trainloss: %f' % (epoch,mae,mse,train_loss)
        log_print(log_text, color='green', attrs=['bold'])
        log_text = 'BEST MAE: %0.1f, MSE: %0.1f, BEST MODEL MAE: %s' % (best_mae,mse, best_model_mae)
        log_print(log_text, color='green', attrs=['bold'])
        log_text = 'MAE: %0.1f, BEST MSE: %0.1f, BEST MODEL MSE: %s' % (mae,best_mse, best_model_mse)
        log_print(log_text, color='green', attrs=['bold'])
        log_text = 'trainloss:%f, BEST MODEL MSE: %s' % (best_trainloss, best_model_trainloss)
        log_print(log_text, color='green', attrs=['bold'])
        if use_tensorboard:
