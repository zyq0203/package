...
class CrowdCounter(nn.Module):
    ...
    @property
    def loss(self):
        return self.loss_mse+0.001*self.loss_s
    ...
    def forward(self,  im_data, gt_data=None):        
        ...
        if self.training:                        
            ...
            self.loss_mse,self.loss_s = self.build_loss(density_map, gt_data)
            
        return density_map
    
    def build_loss(self, density_map, gt_data):
        loss_mse = self.loss_fn(density_map, gt_data)
        loss_s = self.loss_ssim(density_map, gt_data)
        return loss_mse, loss_s
...
class SSIM_Loss(_Loss):
    def __init__(self, in_channels=1, size=11, sigma=1.5, size_average=True):
